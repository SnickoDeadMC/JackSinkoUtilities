...
namespace MyBox
{
	...
	public class ConditionalHelpBoxFieldAttribute : PropertyAttribute
	{
		...
		public ConditionalHelpBoxFieldAttribute(string fieldToCheck, string message, bool inverse = false, params object[] compareValues)
		{
			Message = message;
...
namespace MyBox.Internal
{
	...
	[CustomPropertyDrawer(typeof(ConditionalHelpBoxFieldAttribute))]
	public class ConditionalHelpBoxFieldAttributeDrawer : PropertyDrawer
	{
		...
		{
			if (!(attribute is ConditionalHelpBoxFieldAttribute conditional)) return 0;

			Initialize(property);

			var propertyToCheck = ConditionalHelpBoxFieldUtility.FindRelativeProperty(property, conditional.FieldToCheck);
			_toShow = ConditionalHelpBoxFieldUtility.PropertyIsVisible(propertyToCheck, conditional.Inverse, conditional.CompareValues);
			if (!_toShow) return EditorGUI.GetPropertyHeight(property);
		...
		{
            //combining attributes here:
            if (_toShow && attribute is ConditionalHelpBoxFieldAttribute conditional)
				EditorGUILayout.HelpBox(conditional.Message, MessageType.Warning);
		...
		private void Initialize(SerializedProperty property)
		{
			...
			void TryGetCustomAttributeDrawer()
			{
				...
				var secondAttribute = (PropertyAttribute) fieldInfo.GetCustomAttributes(typeof(PropertyAttribute), false)
					.FirstOrDefault(a => !(a is ConditionalHelpBoxFieldAttribute));
	...
	public static class ConditionalHelpBoxFieldUtility
	{
#region Property Is Visible
		...
		public static bool PropertyIsVisible(SerializedProperty property, bool inverse, string[] compareAgainst)
		{
			...
			bool someValueAssigned = asString != "FALSE" && asString != "0" && asString != "(0.0, 0.0, 0.0)" && asString != "(0.0, 0.0)" && asString != "" && asString != "NULL";
		...
		/// </summary>
		public static bool CompareAgainstValues(string propertyValueAsString, string[] compareAgainst, bool handleFlags)
		{
...
#region Find Relative Property
		...
		public static SerializedProperty FindRelativeProperty(SerializedProperty property, string propertyName)
		{
			...
			if (nestedProperty == null)
			{
				...
				var warning = who + "Array fields is not supported by [ConditionalHelpBoxFieldAttribute]. Consider to use <color=blue>CollectionWrapper</color>";
...
#region Behaviour Property Is Visible
		...
		public static bool BehaviourPropertyIsVisible(UnityEngine.Object obj, string propertyName, ConditionalHelpBoxFieldAttribute appliedAttribute)
		{
